...
def reshape_ts(ts, window_size):
    """
    Reshape time series with window of duration
    :para ts: time series with shape nbatch x ndims x T
    :para window_size: window size of duration.
    :return nbatch * (T-window_size+1) x (ndims * T)
    """
    nbatch, ndims, T = ts.shape
    mat = list()
    for batch in range(nbatch):
        for i in range(T - window_size + 1):
            mat.append(ts[batch, :, i:i+window_size].T.reshape(-1))
    mat = np.stack(mat)
    return mat.T


class GassianMIEstimator(object):
    """Estimate the mutual information with Gaussian distribution assumpetion"""
    def __init__(self, T_past, T_future=None):
        """
        :para T_past: the window of duration of past
        :para T_future: the window of duration of future
        """
        self.T_past = T_past
        if T_future is None:
            self.T_future = T_past
        else:
            self.T_future = T_future

    def update_T(self, T_past, T_future):
        self.T_past = T_past
        self.T_future = T_future

    def fit(self, data):
        data_past = reshape_ts(data, self.T_past)
        data_future = reshape_ts(data, self.T_future)
        data_joint = reshape_ts(data, self.T_past + self.T_future)
        sample_cov_past = np.cov(data_past)
        sample_cov_future = np.cov(data_future)
        sample_cov_joint = np.cov(data_joint)
        MI = 0.5 * (np.linalg.slogdet(sample_cov_past)[1] + np.linalg.slogdet(sample_cov_future)[1]
                    - np.linalg.slogdet(sample_cov_joint)[1])
        return MI


class MIEstimator(nn.Module):
    """
    Class of neural estimator of Mutual Information
    """
    def __init__(self, critic_params, data_params, mi_params, proj_dim=None, proj_init=None):
...
class MIEstimator_Trainer:
    def __init__(self,
        ...
        self.trainable_vars = list(self.model.critic.parameters())
        if self.model.baseline is not None:
            self.trainable_vars += list(self.model.baseline.parameters())
        if self.model.proj is not None:
            self.trainable_vars += list(self.model.proj.parameters())
    ...
    def fit(self, dataloader, epochs=100, patience=5, min_delta=0, show_history=False, save_file="res.pickle"):
        """
        ...
        :param epochs: number of maximum epochs.
        :param show_history: all historical tracks.
        :return:
        """
        history_MI = []
        ...
        counter = 0
        min_loss = torch.inf
        for _ in tqdm(range(epochs), disable=self.tqdm_disable):
            if dist.get_rank() == 0:
                MI_epoch = 0
            for i_batch, sample_batch in enumerate(dataloader):
                ...
                if self.model.module.proj is not None:
                    x = self.model.module.proj(x)
                x = x.reshape(batch_size, -1)
                y = y.reshape(batch_size, -1)
                MI_loss = self.model(x, y)
                self.optimizer.zero_grad()
                MI_loss.backward()
                self.optimizer.step()
                dist.reduce(MI_loss, dst=0)
                if dist.get_rank() == 0:
                    loss_mean = MI_loss.detach().cpu().numpy()/dist.get_world_size()
                    if loss_mean < min_loss + min_delta:
                        min_loss = loss_mean
                        counter = 0
                    else:
                        counter += 1
                        if counter >= patience:
                            flag = torch.ones(1).cuda()
                else:
                    pass
                ...
                if flag:
                    if dist.get_rank() == 0:
                        if show_history:
                            print(np.asarray(history_MI))
                            with open(save_file, "wb") as file:
                                pickle.dump((np.asarray(history_MI), -min_loss), file)
                        else:
                            print(-min_loss)
                            with open(save_file, "wb") as file:
                                pickle.dump(-min_loss, file)
                    print("training stop for rank {}".format(dist.get_rank()))
                    return 0
                if dist.get_rank() == 0:
                    MI_epoch -= loss_mean
            if dist.get_rank() == 0:
...
if __name__ == "__main__":
    ...
    print(reshape_ts(ts, window_size=4))
    pass
