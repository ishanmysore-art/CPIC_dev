...
config_file_dict = {
        'lorenz_stochastic_infonce_sparse': './config/config_lorenz_stochastic_infonce_sparse.ini',
    }
...
def load_sparse_cpic_params(cfg):
    ...
    gamma = cfg.getfloat('Hyperparameters', 'gamma')
...
if __name__ == "__main__":
    ...
    ydim, xdim, T, hidden_dim, beta, mi_params, critic_params, baseline_params, predictive_space, gamma = \
        (lambda p: (p["ydim"], p["xdim"], p["T"], p["hidden_dim"], p["beta"],
                    p["mi_params"], p["critic_params"], p["baseline_params"], p["predictive_space"], p["gamma"]))(load_sparse_cpic_params(cfg))
    ...
    for snr_val, X_pca_trans, X_dca_trans, X_noisy in zip(snr_vals, X_pca_trans_dset, X_dca_trans_dset, X_noisy_dset):
        ...
        loss, _, _, _ = sparse_cpic.fit(X=train_data, 
        ...
        if encoder_type == "conv_spatiotemporal":
            ...
            encoded_windows = sparse_cpic.encode(past_tensor) # (N-T, T, ydim)
            encoded_repr = encoded_windows[:, -1, :] # (N-T, ydim)
            X_sparse_CPIC_trans = aligned_encoded_mean = linear_alignment(encoded_repr.cpu().detach().numpy(), X_true_r2)
        else:
            encoded_repr = encoded_mean = sparse_cpic.encode(torch.from_numpy(X_noisy).to(device))
            X_sparse_CPIC_trans = aligned_encoded_mean = linear_alignment(encoded_mean.cpu().detach().numpy(), X_dynamics)

        R2_PCA = compute_R2(X_pca_trans, X_dynamics)
        R2_DCA = compute_R2(X_dca_trans, X_dynamics)
        R2_sparse_CPIC = compute_R2(aligned_encoded_mean, X_true_r2)
        print("R2(PCA): {}, R2(DCA): {}, R2(sparse CPIC): {}".format(R2_PCA, R2_DCA, R2_sparse_CPIC))
        R2_metrics.append([R2_PCA, R2_DCA, R2_sparse_CPIC])
        losses.append(loss)
        inferred_sparse_CPIC_trials.append(X_sparse_CPIC_trans)
        if do_vis_latent_trials:
            plot_latent_trials(X_dynamics, X_pca_trans, X_dca_trans, X_sparse_CPIC_trans, num_vis, snr_val=snr_val, save_dir=saved_root)
    
    # save the results
    R2_metrics = np.stack(R2_metrics)
    seed_str = f"_seed{args.seed}" if args.seed is not None else ""
    # save the encoded representations
    with open(f"{saved_root}/encoded_representations{seed_str}.pkl", "wb") as f:
        pickle.dump({"encoded_representations": encoded_repr}, f)
    ...
    with open(f"{saved_root}/inferred_trials{seed_str}.pkl", "wb") as f:
        pickle.dump({"inferred_sparse_CPIC_trials": inferred_sparse_CPIC_trials, "snr_vals": snr_vals}, f)
